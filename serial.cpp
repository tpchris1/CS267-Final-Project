...
// Get bin id for current particle
int inline get_bin_id(particle_t& particle) {
    int x, y;
    x = particle.x / bin_size;
    y = particle.y / bin_size;
    if (x == bin_row_count) { // if on the edge of entire grid, then belong to last bin 
        x--;
    }
    if (y == bin_row_count) { // if on the edge of entire grid, then belong to last bin 
        y--;
    }
    return y * bin_row_count + x; // store in 1d fashion, retireve through this method
}
